<div class="row">
    <div class="col-md-6">
        <h3><?= $title ?></h3>
        <form action="<?= site_url('type/save'); ?>" method="post">
            <input type="hidden" name="is_edit" value="<?= $is_edit ?>">
            <input type="hidden" name="id" value="<?= ($is_edit == 1) ? $result[0]->id_type : '' ?>">
            <div class="form-group">
                <label class="form-label" for="type">Nama Type : </label>
                <input class="form-control" type="text" name="type" required="required" value="<?= ($is_edit == 1) ? $result[0]->nm_type : '' ?>">
            </div>
            <div class="form-group">
...
<script>
    $('#cancel').click(function () {
        window.location.replace("<?= site_url('type'); ?>");
    });
</script>